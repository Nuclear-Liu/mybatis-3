...
public class PooledDataSource implements DataSource {
  ...
  /**
   * 当连接超时 poolPingConnectionsNotUsedFor 毫秒未使用时，会发送一次测试 SQL 语句，检车连接是否正常.
   */
  protected int poolPingConnectionsNotUsedFor;

  /**
   * 根据数据库 URL 、用户名和密码生成一个 hash 值，判断该连接是否是当前数据库的连接.
   */
    ...
    while (conn == null) {
      ...
      try {
        if (!state.idleConnections.isEmpty()) { /* 检查是否有空闲连接 */
          ...
          /**
           * 连接池中有空闲连接，直接返回空闲可用的连接.
           */
          conn = state.idleConnections.remove(0);
          if (log.isDebugEnabled()) {
            log.debug("Checked out connection " + conn.getRealHashCode() + " from pool.");
          }
        } else if (state.activeConnections
            .size() < poolMaximumActiveConnections) { /* 当前连接池中没有空闲连接，并且活动连接数没有超过连接池设置的阈值 */
          ...
          /**
           * 当前连接池中没有空闲连接，并且活跃连接数已经到达连接池设置的阈值，判断有没有连接超时的连接：如果有处理超时连接，没有就阻塞等待.
           * <p/>
           * 不可以创建新连接
           */
          ...
          if (longestCheckoutTime > poolMaximumCheckoutTime) {
            ...
            /**
             * 创建新的连接池连接对象，服用了上次的连接对象 {@link PooledConnection#realConnection}.
             */
